...
(defun emt-ensure ()
  ...
  (unless emt--lib-loaded
    (let ((just-downloaded nil))
      (unless (file-exists-p emt-lib-path)
        (if (yes-or-no-p "EMT module not found. Download ewt-rs pre-built from GitHub? ")
            (progn (emt-download-module) (setq just-downloaded t))
          (error "EMT module cannot be loaded")))
      (if (and just-downloaded (eq system-type 'gnu/linux))
          (message "EMT module downloaded to %s. Please restart Emacs." emt-lib-path)
